				...
				.ajax({
					...
					success : function(json) {
						...
						$
								.each(
										json,
										function(index, item) { //index는 무엇인지?
											str += "<tr>";
											str += "<td>" + item.notice_num
													+ "</td>";
											str += "<td>"
													+ "<a href='noticeDetail?notice_num="
													+ item.notice_num + "'>"
													+ item.title + "</a>"
													+ "</td>";
											str += "<td>" + item.credate
													+ "</td>";
											str += "<td>" + item.writerName
													+ "</td>";
											str += "<td>" + item.count
													+ "</td>";
											str += "</tr>";
							...
							if (startpage >= 1) {
								$
										.ajax({
											...
											data : "dept=" + dept + "&checked="
													+ checked, //보낼때의 값이다 index/1?dept= 의 주소에 값을 넣을때 쓴다
											dataType : "json",
											success : function(json) {
												$("#list").empty();
												var str = "";
						...
						<div class="box">
							<div class="box-header">
								...
								<span style="float: right !important; margin: 10px;">
									<button value="O" id="open"
										class="btn btn-default btn-flat disabled"
										user="${user.departmentId}">공지사항</button> <span>
										<button value="D" id="development"
											class="btn btn-primary btn-flat disabled"
											user="${user.departmentId}">개발</button>
								</span> <span>
										<button value="M" id="manage"
											class="btn btn-success btn-flat disabled"
											user="${user.departmentId}">유지보수</button>
								</span> <span>
										<button value="R" id="run"
											class="btn btn-info btn-flat disabled"
											user="${user.departmentId}">경영</button>
										<button value="P" id="person"
											class="btn btn-danger btn-flat disabled"
											user="${user.departmentId}">인사회계</button>
										<button value="S" id="salary"
											class="btn btn-warning btn-flat disabled"
											user="${user.departmentId}">영업</button>
								</span>
								</span>
							</div>
							<!-- /.box-header -->
							<div class="box-body">
								<table class="table table-bordered">
									...
									<tbody id="list">
										<!--tbody는 ajax에서 쓸때 어디부분했는지를 알기위해  -->
									</tbody>
								</table>
								<!-- /.box-body -->
